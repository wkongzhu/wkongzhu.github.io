...
- 另外一类是乘法操作，也就是 $x\cdot y\ (mod\ N), (0\leqslant x,y\lt N)$, 那么
    ...
    def Zn_Mul(x, y):
        ...
        '''
        t = 0
        for i in reversed(range(len(bin(y))-2)): # len(bin) -2 等于y的二进制数的总位数, i从高位开始
            t = (t + t) % N
...
$$\qquad=(y_{0}\cdot x\cdot 1)+(y_{1}\cdot x\cdot 10)+(y_{2}\cdot x\cdot 100)$$
...
$$\qquad=\frac{(y_{0}\cdot x\cdot 1)+(y_{1}\cdot x\cdot 10)+(y_{2}\cdot x\cdot 100)}{1000}$$

$$\qquad=\frac{(y_{0}\cdot x)}{1000}+\frac{(y_{1}\cdot x)}{100}+\frac{(y_{2}\cdot x)}{10}$$

$$\qquad=((((((0)+y_0\cdot x)/10)+y_1\cdot x)/10)+y_2\cdot x)/10$$
...
我们知道剩余类集合 $Z_{667}=\{0,1 \cdots 666\}$, 是不存在小数的, 而如果我们采用自然数集的计算方式的话,
...
这个操作是不用除法的。等等, 算法中明明有个除1000的操作, 你骗谁呢。不知道你有没有发现,
